...
public class NetworkManager : MonoBehaviour
{
	...
	private void HandleKill(ISFSObject dt) {
		...
		if (userId != smartFox.MySelf.Id) {
			...
			if (killerId == smartFox.MySelf.Id) {
				SoundManager.Instance.PlayKillEnemy(PlayerManager.Instance.GetPlayerObject().GetComponent<AudioSource>());
			}
		}
		else {
	...
	private void HandleHealthChange(ISFSObject dt) {
		...
		if (userId == smartFox.MySelf.Id) {
			if (health<GameHUD.Instance.Health) {
				SoundManager.Instance.PlayDamage(PlayerManager.Instance.GetPlayerObject().GetComponent<AudioSource>());
			}
	...
	private void HandleRemoveItem(ISFSObject dt) {
		...
		if (playerId == smartFox.MySelf.Id) {
			if (type == "HealthPack") {
				SoundManager.Instance.PlayPickupHealthPack(PlayerManager.Instance.GetPlayerObject().GetComponent<AudioSource>());				
			} 
			else if (type == "Ammo") {
				SoundManager.Instance.PlayPickupAmmo(PlayerManager.Instance.GetPlayerObject().GetComponent<AudioSource>());							
			}
		}
	...
	private void HandleReload(ISFSObject dt) {
		int userId = dt.GetInt("id");
		if (userId != smartFox.MySelf.Id) {
			SoundManager.Instance.PlayReload(PlayerManager.Instance.GetRecipient(userId).GetComponent<AudioSource>());
			...
			SoundManager.Instance.PlayReload(obj.GetComponent<AudioSource>());
